...
class ResNet(nn.Module):
    ...
    def forward(self, x, k, theta=None, labels=None):
        x = self.conv1(x)
        ...
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        features_numpy = x.data.cpu().numpy()
        features = x
        threshold = int(config.train_batch_size * 0.2)
        if labels is not None:
            adj, theta = graph_fun.alter_knn_and_radius_graph1(features_numpy, k, labels, None, config.class_num, limit_radius=True)
        else:
            adj = graph_fun.knn_and_radius_graph(features_numpy, k, theta, None, None, limit_radius=False)
        ...
        x = self.gc2(x, A, D)
        x_label = x[:threshold]
        x_non_label = x[threshold:]
        return x, features, x_label, x_non_label, theta, adj_tensor
